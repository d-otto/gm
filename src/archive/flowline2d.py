...
#           end
    ...
    for j in range(0, nxs-1):  # this is a kloudge -fix sometime
        if j == 0:
            ...
            Qp[0] = -(dhdx + dzdx)**3 * h_ave**4 * (rho * g)**3 * (fd * h_ave + fs / h_ave)  # flux at plus half grid point
    ...
    if t / deltout == np.floor(t / deltout):
        ...
        bal_out[idx_out] = np.trapz(bal[:np.argmax(h[h>1])])  # should add up all the mass up to the edge, and be zero in equilibrium (nearly zero)
        idx_out = idx_out + 1
